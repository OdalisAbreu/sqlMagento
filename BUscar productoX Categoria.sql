SELECT   DISTINCT P.product_id, sku.sku, `name`.`value` AS 'Name', C.entity_id, C.`value` AS 'Category', short.`value` AS Short, `long`.`value` AS 'Long description',
kw.value AS 'Meta Keywords', brand.`value` AS 'Brand', `pq`.`stock_qty` AS 'Qty', `pq`.`vendor_price` AS 'price',
CONCAT( 'https://www.shopmundo.com/media/catalog/product',`product_image`.value) AS 'IMG', W.value AS 'Weight', model.`value` AS 'Model', `status`.`value` AS 'status'
FROM 
