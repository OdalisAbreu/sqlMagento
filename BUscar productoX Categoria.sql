SELECT   DISTINCT P.product_id, sku.sku, `name`.`value` AS 'name', C.entity_id, C.`value`, `status`.`value` AS 'status', short.`value` AS Short, `long`.`value` AS 'Long description',kw.value AS 'Meta Keywords' ,
 `pq`.`stock_qty` AS 'Qty', `pq`.`vendor_price` AS 'price', CONCAT( 'https://www.shopmundo.com/media/catalog/product',`product_image`.value) AS 'IMG', W.value AS 'Weight', model.`value` AS 'Model', brand.`value` AS 'brand'
...
WHERE  
C.attribute_id = 41 AND  C.store_id = '0' AND C.entity_id = 1146 AND `name`.attribute_id = 71 AND short.`attribute_id` = 73 AND `long`.`attribute_id` = 72 AND `kw`.`attribute_id` = 83 AND `product_image`.`attribute_id` = 85 ;